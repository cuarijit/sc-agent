...
@router.get("/api/inventory-allocation/runs/{run_id}/steps")
def list_allocation_run_steps(
    run_id: str,
    service: AgentConfigService = Depends(get_agent_config_service),
) -> list[dict[str, object]]:
    # Reuses the shared AgentRunStepArtifact table — step recording is
    # agent-agnostic, so the diagnostic loader works here as-is.
    from ..services.inventory_diagnostic.step_recorder import load_step_artifacts

    return load_step_artifacts(service.db, run_id)


@router.post("/api/inventory-allocation/query")
def inventory_allocation_query(
    payload: dict[str, object],
    service: AgentConfigService = Depends(get_agent_config_service),
) -> dict[str, object]:
    from ..services.inventory_allocation import InventoryAllocationRunner
    ...
    try:
        result = InventoryAllocationRunner(service.db).run(
            message=message,
